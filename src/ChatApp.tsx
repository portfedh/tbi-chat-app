  ...
  const saveCurrentChat = () => {
    if (chatHistory.length > 0) {
      const chatToSave = {
        ...
        title: currentChatTitle || chatHistory[0].content.substring(0, 30) + '...',
      ...
      setSavedChats(prevChats => {
        ...
        const filteredChats = currentChatId !== 'new' 
  ...
  const handlePasteText = () => {
    navigator.clipboard.readText().then(text => {
      if (text) {
        ...
        setDocuments(prev => [...prev, pastedDoc]);
      }
    }).catch(err => {
      setError("Failed to read clipboard. Please paste text manually.");
        ...
        <div className="w-64 bg-gray-100 p-4 flex flex-col border-r">
          ...
          <div className="mb-4">
            <div className="flex justify-between items-center mb-2">
              <label className="block text-sm font-medium text-gray-700">Saved Chats</label>
              <button 
                  ...
                  {savedChats.map(chat => (
                    <li 
                      ...
                      className={`text-sm p-2 rounded cursor-pointer flex justify-between items-center ${
                        currentChatId === chat.id ? 'bg-blue-100' : 'bg-white hover:bg-gray-200'
                      }`}
                    >
          ...
          {documents.length > 0 && (
            <div className="flex-1 overflow-y-auto">
              <p className="text-sm font-medium text-gray-700 mb-1">Uploaded Documents:</p>
              <ul className="space-y-2">
                {documents.map((doc, index) => (
                  <li key={index} className="text-sm bg-white p-2 rounded border flex justify-between items-center">
          ...
          <div className="flex-1 p-4 overflow-y-auto">
            {chatHistory.length === 0 ? (
              <div className="h-full flex items-center justify-center text-gray-500">
                <p>Start a conversation by sending a prompt</p>
              </div>
            ) : (
